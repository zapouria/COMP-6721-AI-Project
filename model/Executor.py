...
class Executor:
    ...
    def training_model_executor(self, data_src, num_iters):
        ...
        for iter in tqdm(range(0, num_iters), total=num_iters, desc="train model"):
            self.network.train()
            for item in data_src:
                result = self.network(item[0])
                calc_loss = self.entropy_loss(result, item[1]).item()
                all_loss_vals.append(calc_loss)
                self.gd_reducer.zero_grad()
                calc_loss.backward()
                self.gd_reducer.step()
            total_iter_loss = sum(each_iter_loss)
            covered_so_far = len(each_iter_loss)
            avg_loss = total_iter_loss / covered_so_far
            each_iter_loss.append(each_iter_loss)
